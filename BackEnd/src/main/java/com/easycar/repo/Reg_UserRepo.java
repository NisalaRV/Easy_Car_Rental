...
public interface Reg_UserRepo extends JpaRepository<Reg_User,String> {
    ...
    @Query(value = "SELECT reg_user.user_Id,reg_user.firstName,reg_user.lastName,reg_user.lastName,reg_user.contact_No,reg_user.address,reg_user.email1,reg_user.nic,reg_user.license_No,reg_user.nic_Img,reg_user.license_Img FROM Reg_User JOIN  user u on u.user_Id = reg_user.user_Id where user_Name=?1", nativeQuery = true)
